...
Phase 2 is implemented in stacked PR #2, branch `test/concurrent-delivery-recovery`, based on PR #1. It adds process-isolated races and SIGKILL recovery tests without production protocol changes. Before reporting verification or moving on, inspect the latest PR #2 CI: the integration job runs both `make integration` and three shuffled/race-instrumented repetitions via `make reliability`. Local build/unit/race/vet/lint and integration compilation passed; these alone do not verify the PostgreSQL scenarios. Both PRs remain unmerged unless GitHub says otherwise.

Next after the phase 2 CI gate: phase 3, GPT-6 Astra / high. Add supplier B with independent durable state and disjoint stock, controlled faults, timeout-after-issuance replay, jitter and safe fallback only after durable refusal. Reconciliation and large-catalog measurements remain later phases. Reviews and limitations are in `docs/reviews/phase-1b.md` and `docs/reviews/phase-2.md`.
